...
def getWordScore(word, n):
    ...
    points = 0
    for w in word:
        if w in SCRABBLE_LETTER_VALUES:
            point += SCRABBLE_LETTER_VALUES[w]
        points = point * l
    if l == n:
        return points+50
    else:
...
def isValidWord(word, hand, wordList):
    ...
    # if cWord == True and cHand == True and same ==True:
    #     result = True
    # return result
...
def playGame(wordList):
    ...
    while True:
        check = str(input('Enter n to deal a new hand, r to replay the last hand, or e to end game: '))
        
        if check == 'e':
            break
        
        elif check == 'r' and count == 0:
